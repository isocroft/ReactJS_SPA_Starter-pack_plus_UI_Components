...
export const usePageDataLoader = (location: Location<unknown>) => {
  ...
  const query = {
    data: [
      { id: 789, make: "Volvo", color: "Red" },
      { id: 567, make: "Mercedes", color: "Black" },
    ],
    isLoading: false,
    isSuccess: true,
    error: null,
  ...
  const anotherQueryResult = useRegionDataLoader({
    vehicleIds: query.data.map((datum) => datum.id),
  });

  return anotherQueryResult;
};
